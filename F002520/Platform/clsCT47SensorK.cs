    ...
    class clsCT47SensorK : clsSensorKBase
    {
            ...
            try
            {
                ...
                if (DeviceNameList.Length > 1)
                {
                    ...
                    for (int i = 0; i < DeviceNameList.Length; i++)
                    {
                        ...
                        Match match = Regex.Match(deviceName, @"(CT47)");
                        if (match.Success)
                        {
        ...
        public override bool TestMoveDamBoardUp(ref string strErrorMessage)
        {
            ...
            try
            {
                ...
                DisplayMessage("Move the DamBoard to Position: " + dPosition.ToString() + "cm");
        ...
        public override bool TestPSensorCalibration(ref string strErrorMessage)
        {
            ...
            try
            {
                ...
                // Far Calibration CMD
                strFarRunCmd = GetTestItemParameter(strTestItem, "FarCalibrationCmd");
                if (string.IsNullOrWhiteSpace(strFarRunCmd))
                {
                    strErrorMessage = "Fail to parse FarCalibrationCmd parameter !";
                    return false;
                }
                DisplayMessage("Param FarCalibrationCmd: " + strFarRunCmd);

                // Near Calibration CMD
                strNearRunCmd = GetTestItemParameter(strTestItem, "NearCalibrationCmd");
                if (string.IsNullOrWhiteSpace(strNearRunCmd))
                {
                    strErrorMessage = "Fail to parse NearCalibrationCmd parameter !";
                    return false;
                }
                DisplayMessage("Param NearCalibrationCmd: " + strNearRunCmd);
                 ...
                 * 1. Move the DamBoard to Far Position (15cm), Run PSensor Calibration Cmd.
                 * 2. Move the DamBoard to Near Position (5cm), Run PSensor Calibration Cmd.
                 * 
                 * After Calibration, The Two Threshold Value Must be Changed.
                 *******************************************************************************/

                // Far Position Calibration
                if (PSensorFarCalibration(strFarPosition, strFarRunCmd, ref strErrorMessage) == false)
                {
                    strErrorMessage = "PSensor Far-Position Calibration Fail !!!";
                    return false;
                }
                DisplayMessage("PSensor Far-Position Calibration Successful.");
       
                // Near Position Calibration
                if (PSensorNearCalibration(strNearPosition, strNearRunCmd, ref strErrorMessage) == false)
                {
                    strErrorMessage = "PSensor Near Position Calibration Fail !!!";
                    return false;
                }
                DisplayMessage("PSensor Near-Position Calibration Successful.");
                ...
                {
                    strErrorMessage = "Before and After calibration, PROXIMITY_CALIBRATION_EXTEND value not changed !!!";
                    DisplayMessage(strErrorMessage, "ERROR");
                    return false;
                }
        ...
        private bool ScreenOffAction_OLD(ref string strErrorMessage)
        {
            strErrorMessage = "";
            bool bRes = false;
            string strCmd = "";
        ...
        private bool ScreenOffAction(ref string strErrorMessage)
        {
            ...
            try
            {
                DisplayMessage("Check Screen Status.");
                strCmd = "adb shell \"dumpsys deviceidle | grep mScreenOn\"";
                DisplayMessage("Check Screen Status, Send Cmd: " + strCmd);
                bRes = clsProcess.ExcuteCmd(strCmd, 500, ref strResult);

                DisplayMessage("Response: " + strResult);
                if (strResult.IndexOf("mScreenOn=false", StringComparison.OrdinalIgnoreCase) != -1)
                {
                    DisplayMessage("The Screen Already Turn off !");
                    return true;
                }
        ...
        private bool ScreenOnAction(ref string strErrorMessage)
        {
            ...
            try
            {
                ...
                DisplayMessage("Response: " + strResult);
                if (strResult.IndexOf("mScreenOn=true", StringComparison.OrdinalIgnoreCase) != -1)
                {
                ...
                strCmd = "adb shell input keyevent 82";
        ...
        private bool PSensorFarCalibration(string strFarPosition, string strRunCmd, ref string strErrorMessage)
        {
            strErrorMessage = "";
            bool bRes = false;
            bool bFlag = false;
            ...
            try
            {
                #region Move DamBoard to Far Position (15 cm)

                double dPosition = Convert.ToDouble(strFarPosition);
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        strErrorMessage = "Fail to do PSensor Far-Position Calibration !!!";
                        bFlag = false;
                        clsUtil.Dly(3.0);
                        continue;
                    }
                    else if (strResult.IndexOf("calibration PASS", StringComparison.OrdinalIgnoreCase) != -1)   // calibration PASS
                    {
                        bFlag = true;
                        break;
                    }
        ...
        private bool PSensorNearCalibration(string strNearPosition, string strRunCmd, ref string strErrorMessage)
        {
            strErrorMessage = "";
            bool bRes = false;
            bool bFlag = false;
            ...
            try
            {
                #region Move DamBoard to Near-Position (5 cm)

                double dPosition = Convert.ToDouble(strNearPosition);
                double dHome = Convert.ToDouble(clsConfigHelper.servoMotor.Home);
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    if (strResult.IndexOf("FAIL", StringComparison.OrdinalIgnoreCase) != -1)    // Somewhere appear fail
                    {
                        strErrorMessage = "Fail to do PSensor Near-Position Calibration !!!";
                        bFlag = false;
                        clsUtil.Dly(3.0);
                        continue;
                    }
        ...
        private bool CheckNearPositionFunction(string strNearPosition, string PSENSOR_GET_LOG, ref string strErrorMessage)
        {
            ...
            try
            {
                ...
                #region Move DamBoard to Near-Position (2.5 cm)
